...
public interface Order1Repository extends JpaRepository<Order, Long> {
    ...
    @Query("UPDATE OrderItem oi " +
           ...
           "WHERE oi.order.id = :orderId AND oi.product.productId = :productId")
    void updateOrderItemQuantity(@Param("orderId") Long orderId,
                                 @Param("productId") int productId,
                                 @Param("quantity") int quantity);

    Optional<Order> findByOrderItemsProductProductIdAndId(int productId, Long orderId);

    @Modifying
    @Query("DELETE FROM OrderItem oi WHERE oi.order.id = :orderId AND oi.product.productId = :productId")
